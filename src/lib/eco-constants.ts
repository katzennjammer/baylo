...
// Eco-Points awarded per completed task. VERIFIED_SWAP and SAFEZONE_MEETUP are
// repeatable (awarded once per trade); the rest are one-time.

export type EcoTaskKey =
  | "VERIFY_ACCOUNT"
  | "COMPLETE_PROFILE"
  | "FIRST_LISTING"
  | "VERIFIED_SWAP"
...
export const ECO_TASK_POINTS: Record<EcoTaskKey, number> = {
  VERIFY_ACCOUNT:   10,
  COMPLETE_PROFILE: 10,
  FIRST_LISTING:    15,
  VERIFIED_SWAP:    20,
...
export const ECO_TASK_ORDER: EcoTaskKey[] = [
  "VERIFY_ACCOUNT",
  "COMPLETE_PROFILE",
  "FIRST_LISTING",
  "VERIFIED_SWAP",
...
export interface EcoTasksStatus {
  ecoPoints:      number
  googleVerified: boolean
  tasks:          EcoTaskState[]
}

// Eco-Points recognition ranks — ascending thresholds, display-only.
// Eco-Points are never spendable or convertible to Pasa Points.
export const ECO_RANKS = [
  { min: 0,   label: "Eco Starter" },
  { min: 50,  label: "Eco Mover" },
  { min: 150, label: "Eco Hero" },
  { min: 300, label: "Green Champion" },
] as const

export function getEcoRank(points: number): {
  label: string
  next: { label: string; ptsToNext: number } | null
} {
  let idx = 0
  for (let i = 0; i < ECO_RANKS.length; i++) {
    if (points >= ECO_RANKS[i].min) idx = i
  }
  const next = ECO_RANKS[idx + 1]
  return {
    label: ECO_RANKS[idx].label,
    next: next ? { label: next.label, ptsToNext: next.min - points } : null,
  }
}
