...
import { CompactRepBadge, EcoRankBadge } from "@/components/RepBadge";
...
import { factorForCategory, computeGreenScore } from "@/lib/eco-constants";
...
export interface SerializedItem {
  ...
  userId: string;
  tradeCount: number;
  user: { id: string; name: string; avatar: string | null; rating: number; totalTrades: number; ecoPoints?: number };
}
...
interface Props {
  ...
  me: { id: string; name: string; avatar: string | null; points: number };
...
function parsePickup(raw: string | null): { lat: number; lng: number; address: string } | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as { pickup?: { lat: number; lng: number; address?: string } };
    if (!parsed.pickup) return null;
    return { lat: parsed.pickup.lat, lng: parsed.pickup.lng, address: parsed.pickup.address ?? "" };
  } catch { return null; }
}

function parseWanted(raw: string | null): string | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as { wanted?: string | null };
    return parsed.wanted ?? null;
  } catch { return raw; }
}
...
function fmtPts(estimatedValue: number | null): string | null {
  if (estimatedValue == null) return null;
  return `~${phpToPoints(estimatedValue).toLocaleString("en-US")} pts`;
}
...
function itemTags(item: SerializedItem): string[] {
  ...
  if (parsePickup(item.wantedItems)) tags.push("Pickup");
...
function FeaturedHero({ item, onOffer, isOwner, traderOfWeek, trendingCategory }: {
  ...
  const wanted = parseWanted(item.wantedItems);
  const ptsStr = fmtPts(item.estimatedValue);
          ...
          <div className="tp-hero-foot">
            ...
            {ptsStr && (
              <div className="tp-hero-stats">
                <span className="tp-hero-stat">{ptsStr}</span>
              </div>
            )}
            {isOwner ? (
...
function BuildOfferModal({
  ...
  const [alertNeededPts, setAlertNeededPts] = useState(0);
  ...
  const askPts = phpToPoints(listing.estimatedValue ?? 0);
  const offerItemsPts = selectedItems.reduce(
    (s, i) => s + phpToPoints(i.estimatedValue ?? 0),
    0,
  );
  const offerTotal = offerItemsPts + points;
  const pct = askPts > 0 ? Math.min(1, offerTotal / askPts) : offerTotal > 0 ? 1 : 0;
  const gap = Math.max(0, askPts - offerTotal);
  const statusInfo =
  ...
  function coverGap() {
    const needed = Math.max(0, askPts - offerItemsPts);
    if (walletBalance < needed) {
      setAlertNeededPts(needed);
  ...
  async function propose() {
    if (selectedItems.length === 0 && points === 0) return;
    setSubmitting(true);
    try {
      const res = await fetch("/api/offers", {
        ...
        body: JSON.stringify({
          ...
          offeredPoints: points > 0 ? points : null,
          message: null,
        }),
      });
      if (!res.ok) {
  ...
  const pointsExceedBalance = points > walletBalance;
  const canPropose = (selectedItems.length > 0 || points > 0) && !submitting && !pointsExceedBalance;
      ...
      <div className="bom-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bom-receive-card">
              ...
              <div className="bom-receive-info">
                ...
                <span className="bom-sub">
                  ...
                  {askPts > 0 ? ` · ~${askPts.toLocaleString("en-US")} pts value` : ""}
                </span>
          ...
          <div className="bom-offer-col">
            ...
            {selectedItems.length > 0 && (
              <div className="bom-offered-items">
                {selectedItems.map((item) => {
                  ...
                  const itemPts = phpToPoints(item.estimatedValue ?? 0);
                  return (
                    <div key={item.id} className="bom-offered-item">
                      ...
                      <strong>{item.title}</strong>
                      {itemPts > 0 && <span className="bom-item-pts">{itemPts.toLocaleString("en-US")} pts</span>}
                      <button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="bom-shelf-info">
                          <strong>{item.title}</strong>
                          {itemPts > 0 && <span>~{itemPts.toLocaleString("en-US")} pts</span>}
                        </div>
            ...
            <div className="bom-pts-panel">
              <div className="bom-pts-head">
                ...
                <span className="bom-pts-balance">Balance {walletBalance.toLocaleString("en-US")} pts</span>
              </div>
              <div className="bom-pts-stepper">
                <button className="bom-pts-step" onClick={() => setPoints((p) => Math.max(0, p - 50))}>−</button>
                <div className="bom-pts-wrap">
                  ...
                  />
                  <span className="bom-pts-suffix">pts</span>
                </div>
                <button className="bom-pts-step" onClick={() => setPoints((p) => Math.min(walletBalance, p + 50))}>+</button>
                {gap > 0 && (
                  <button className="bom-cover-btn" onClick={coverGap}>
                    Cover {gap.toLocaleString("en-US")} pts
                  </button>
                )}
              </div>
              {pointsExceedBalance && (
                <div style={{ marginTop: 6, fontSize: 12, color: "#dc2626", display: "flex", alignItems: "center", gap: 6 }}>
                  <span>You only have {walletBalance.toLocaleString("en-US")} pts available.</span>
                  <button type="button"
              ...
              {walletBalance === 0 && !pointsExceedBalance && (
        ...
        <div className="bom-foot">
          ...
          <div className="bom-status-sec">
            ...
            <div className="bom-status-vs">your offer vs ~{askPts.toLocaleString("en-US")} pts</div>
            <div className="bom-status-pts">{offerTotal.toLocaleString("en-US")} pts</div>
          </div>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.6, marginBottom: 22 }}>
              You need {alertNeededPts.toLocaleString("en-US")} pts but only have {walletBalance.toLocaleString("en-US")} pts. Top up your wallet to continue.
            </div>
...
export default function TradeplaceClient({ items: initialItems, me, followReqCount, notifs, messages, weeklyTrades, initialOpen, initialEdit, traderOfWeek, trendingCategory, recentTrades }: Props) {
  ...
  const sorted = useMemo(() => {
    const arr = [...filtered];
    if (sortBy === "value_high") arr.sort((a, b) => (b.estimatedValue ?? 0) - (a.estimatedValue ?? 0));
    if (sortBy === "value_low")  arr.sort((a, b) => (a.estimatedValue ?? 0) - (b.estimatedValue ?? 0));
    return arr;
  }, [filtered, sortBy]);
  ...
  const mapPins = useMemo<ListingPin[]>(() =>
    filtered.flatMap((item) => {
      if (!item.user) return [];
      const pickup = parsePickup(item.wantedItems);
      if (!pickup) return [];
      const imgs = parseImages(item.images);
      const wanted = parseWanted(item.wantedItems);
      return [{
        id: item.id, title: item.title, imageUrl: imgs[0] ?? null,
        estimatedValue: item.estimatedValue, lat: pickup.lat, lng: pickup.lng,
  ...
  return (
    ...
    >
                            ...
                            <div className="tp-card-media">
                              ...
                              {ptsStr && (
                                <div className="tp-media-foot">
                                  <span className="tp-val-pill">{ptsStr}</span>
                                </div>
                              )}
                            </div>
      ...
      {selected && (() => {
        ...
        const pickup  = parsePickup(selected.wantedItems);
        const ptsStr  = fmtPts(selected.estimatedValue);
              ...
              <div className="tp-detail-body">
                ...
                <div className="tp-detail-give">
                  ...
                  <div className="tp-detail-gg want">
                    <span>THEY WANT</span>
                    <strong>{parseWanted(selected.wantedItems) ?? "Open to offers"}</strong>
                  </div>
                </div>
                ...
                <div className="tp-detail-trader">
                  ...
                  <div className="tp-detail-trader-id">
                    ...
                    <CompactRepBadge
                      ...
                      size="md"
                    />
                    <EcoRankBadge ecoPoints={selected.user?.ecoPoints ?? 0} size="sm" />
                  </div>
                  {!isOwner && (
                    <button
