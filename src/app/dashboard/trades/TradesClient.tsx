...
import { isPointsOnlyTrade, formatTradeTitleStr } from "@/lib/trade-format"
...
// ── PointsThumb (used in HistoryRow for points-only trades) ──────────────────

function PointsThumb({ points, size = 80 }: { points: number; size?: number }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flex: 1, minWidth: 0 }}>
      <div style={{
        ...
        <Icon name="wallet" size={Math.round(size * 0.3)} />
      </div>
      <span style={{
...
function TradeCard({ trade, myId, onConfirm, onCompleted, onCancel, onHide, onOpenChat }: {
  ...
  const pointsOnly = isPointsOnlyTrade(trade.offeredItem.id, trade.requestedItem.id)
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4, gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 4, minWidth: 0 }}>
            {trade.myItemPoints != null && trade.myItemPoints > 0 ? (
              <>
                <span style={{ fontSize: 22, fontWeight: 700, color: "#1A1A1A", lineHeight: 1 }}>
                  {trade.myItemPoints.toLocaleString()}
                </span>
                <span style={{ fontSize: 13, color: "#6B7280", fontWeight: 500 }}>pts</span>
              </>
            ) : (
...
function OfferRow({ offer, onAccept, onDecline, loadingId }: {
  ...
  return (
    ...
    >
      ...
      {(offer.fairness || (offer.offeredPoints && offer.offeredPoints > 0)) && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          ...
          {offer.offeredPoints && offer.offeredPoints > 0 && (
            <span style={{
              display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 9px",
              ...
              +{offer.offeredPoints.toLocaleString()} pts
            </span>
          )}
        </div>
      )}
...
function HistoryRow({ trade, myId, onHide, onRate }: {
  ...
  return (
    ...
    >
      ...
      {(() => {
        const histPointsOnly = isPointsOnlyTrade(trade.offeredItem.id, trade.requestedItem.id)
        return (
          <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
            {histPointsOnly && (trade.offeredPoints ?? 0) > 0 ? (
              <PointsThumb points={trade.offeredPoints!} size={56} />
            ) : (
...
export default function TradesClient({
  ...
  return (
    <div className="baylo layout-classic" data-theme={dark ? "dark" : "light"} data-density="regular" style={rootStyle}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, paddingLeft: 12 }}>
              <div className="wallet-pill" style={{ cursor: "default" }}>
                <Icon name="wallet" size={14} />
                <span>{me.points.toLocaleString("en-US")} pts</span>
              </div>
              <button
      ...
      {/* ── Swap confirm modal ──────────────────────────────────────────────── */}
      {confirmTrade && (() => {
        const ctPointsOnly = isPointsOnlyTrade(confirmTrade.offeredItem.id, confirmTrade.requestedItem.id)
        const ctOfferedTitle = ctPointsOnly && (confirmTrade.offeredPoints ?? 0) > 0
          ? `${confirmTrade.offeredPoints!.toLocaleString()} pts`
