...
import { CO2_PER_CATEGORY, computeImpactData } from "@/lib/eco-constants"
import { phpToPoints } from "@/lib/wallet-config"
import { isPointsOnlyTrade } from "@/lib/trade-format"
...
export default async function TradesPage() {
  ...
  const [user, trades, rawOffers, rawMsgs, rawNotifs, followReqCount, weeklyCount, weeklyTradesRaw, trendingRaw] = await Promise.all([
    prisma.user.findUnique({
      where: { id: myId },
      select: { name: true, avatar: true, points: true },
    }),
    prisma.tradeRequest.findMany({
      ...
      select: {
        ...
        offeredItem:   { select: { id: true, title: true, images: true, category: true, status: true, estimatedValue: true } },
        requestedItem: { select: { id: true, title: true, images: true, category: true, status: true, estimatedValue: true } },
    ...
    prisma.offer.findMany({
      ...
      include: {
        post:   { select: { id: true, title: true, images: true, estimatedValue: true } },
  ...
  // Collect all offered item IDs so we can fetch their details (title, image, estimatedValue)
  ...
  // ── Points-only: look up offeredPoints from the accepted Offer ───────────
  // For points-only trades, offeredItemId === requestedItemId (placeholder).
  // We recover the points amount by matching the accepted Offer by (senderId, postId).
  const pointsOnlyTrades = trades.filter((t) => isPointsOnlyTrade(t.offeredItemId, t.requestedItemId))
  const offeredPointsMap = new Map<string, number>() // tradeId → points

  if (pointsOnlyTrades.length > 0) {
    const offerRecords = await prisma.offer.findMany({
      where: {
        OR: pointsOnlyTrades.map((t) => ({
          senderId: t.senderId,
      ...
      select: { senderId: true, postId: true, offeredPoints: true },
    })
    const offerMap = new Map(offerRecords.map((o) => [`${o.senderId}:${o.postId}`, o.offeredPoints ?? 0]))
    for (const t of pointsOnlyTrades) {
      const pts = offerMap.get(`${t.senderId}:${t.requestedItemId}`)
      if (pts) offeredPointsMap.set(t.id, pts)
    }
  }
  ...
  const serializedTrades = trades.map((t) => {
    ...
    // Points value of "my item" (the item I'm trading away)
    const myItemRaw    = t.senderId === myId ? t.offeredItem   : t.requestedItem
    const myItemPoints = myItemRaw.estimatedValue != null ? phpToPoints(myItemRaw.estimatedValue) : null
    ...
    return {
      ...
      offeredItem:   {
        ...
        estimatedValue: t.offeredItem.estimatedValue ?? null,
      },
      requestedItem: {
        ...
        estimatedValue: t.requestedItem.estimatedValue ?? null,
      },
      ...
      co2Saved,
      myItemPoints,
      offeredPoints: offeredPointsMap.get(t.id) ?? null,
  ...
  const serializedOffers = rawOffers.map((o) => {
    ...
    const postValue      = o.post.estimatedValue
    const offeredValue   = firstItemDetails?.estimatedValue ?? null
    ...
    return {
      ...
      offeredPoints: o.offeredPoints ?? null,
      fairness,
    }
  })
  ...
  return (
    <TradesClient
      me={{ name: user.name, handle, avatar: user.avatar ?? null, points: user.points }}
