...
import { ECO_TASK_POINTS, getEcoRank, type ImpactData, type EcoTaskKey, type EcoTasksStatus } from "@/lib/eco-constants";
...
interface Me {
  ...
  streak: number; city: string; greenScore: number; tier: string; points: number;
}
      ...
      result.push({
        ...
        spot: {
          ...
          facts: [
            ...
            ["Points", me.points.toLocaleString() + " pts"],
          ],
        },
      });
    }
...
type ReturnPreference = "points" | "item" | "both";
...
function ComposerModal({ onClose, onPosted }: {
  ...
  const [returnPoints, setReturnPoints] = React.useState(0);
  ...
  const submit = async () => {
    ...
    if (returnType === "points" && returnPoints <= 0) { setError("Enter the points amount you want"); return; }
    if (returnType === "item" && !returnItemText.trim()) { setError("Describe what you’re looking for"); return; }
    if (returnType === "both" && (returnPoints <= 0 || !returnItemText.trim())) {
      setError("Fill in both the item description and points amount"); return;
    }
    ...
    try {
      const res = await fetch("/api/items", {
        ...
        body: JSON.stringify({
          ...
          returnType,
          returnPoints: returnType !== "item" ? returnPoints : null,
          wantedItems: returnType !== "points" ? returnItemText.trim() || null : null,
          localPickup,
      ...
      const wantsLabel =
        returnType === "points" ? `${returnPoints} pts`
        : returnType === "both" ? `${returnItemText} · ${returnPoints} pts`
        : returnItemText;
      ...
      onPosted({
        ...
        pickupLocation: localPickup ? pickupLocation : null,
      });
      onClose();
    } catch (e) {
      ...
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-field">
          ...
          <div className="return-fields">
            ...
            {(returnType === "points" || returnType === "both") && (
              <div className="points-input">
                <button type="button" className="points-step"
                        onClick={() => setReturnPoints((p) => Math.max(0, p - 10))}>−</button>
                <input type="number" min={0} value={returnPoints || ""}
                  onChange={(e) => setReturnPoints(Math.max(0, parseInt(e.target.value) || 0))}
                  placeholder="0 pts" className="modal-input points-field" />
                <button type="button" className="points-step"
                        onClick={() => setReturnPoints((p) => p + 10)}>+</button>
              </div>
            )}
          </div>
        </div>
...
function WantModal({ onClose, onPosted }: {
  ...
  const submit = async () => {
    ...
    if (returnType === "points" && returnPoints <= 0) { setError("Enter the points amount you'll offer"); return; }
    if (returnType === "both" && (returnPoints <= 0 || !returnItemText.trim())) {
      setError("Fill in both the item and points amount"); return;
    }
    if (localPickup && !pickupLocation) { setError("Please select a pickup location on the map"); return; }
    setLoading(true);
    try {
...
const PickupPreviewMap = dynamic(() => import("./PickupMap").then((m) => {
  const C = ({ loc }: { loc: PickupLocation }) => <m.default value={loc} onChange={() => {}} />;
  C.displayName = "PickupPreviewMap";
  return C;
}), { ssr: false });

function PickupBadge({ location }: { location: PickupLocation }) {
  const [open, setOpen] = React.useState(false);
  const area = location.address.split(",")[0];
  return (
    <>
...
function MakeOfferModal({ post, onClose, onOpenChat }: {
  ...
  const { me } = useDash();
  const walletBalance = me.points ?? 0;
  const [myItems, setMyItems] = React.useState<VerifiedItem[]>([]);
  const [itemsLoading, setItemsLoading] = React.useState(false);
  const [selectedItem, setSelectedItem] = React.useState<VerifiedItem | null>(null);
  const [offerType, setOfferType] = React.useState<"item" | "points" | "both">("item");
  const [points, setPoints] = React.useState(0);
  const [message, setMessage] = React.useState("");
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");
  const [showTopUpAlert, setShowTopUpAlert] = React.useState(false);
  ...
  const submit = async () => {
    setError("");
    if (offerType !== "points" && !selectedItem) { setError("Select an item to offer"); return; }
    if (offerType !== "item" && points <= 0) { setError("Enter the points amount"); return; }
    setLoading(true);
    try {
      ...
      const res = await fetch("/api/offers", {
        ...
        body: JSON.stringify({
          postId: post.id,
          offeredItems,
          offeredPoints: offerType !== "item" ? points : null,
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-field">
          ...
          {(offerType === "points" || offerType === "both") && (
            <div style={{ marginTop: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, fontSize: 12, color: "var(--ink-3, #888)" }}>
                <span>Baylo points to offer</span>
                <span>Balance: <strong>{walletBalance.toLocaleString("en-US")} pts</strong></span>
              </div>
              <div className="points-input">
                <button type="button" className="points-step"
                        onClick={() => setPoints((p) => Math.max(0, p - 10))}>−</button>
                <input type="number" min={0} max={walletBalance} value={points || ""}
                  onChange={(e) => setPoints(Math.min(walletBalance, Math.max(0, parseInt(e.target.value) || 0)))}
                  placeholder="0 pts" className="modal-input points-field" />
                <button type="button" className="points-step"
                        onClick={() => {
                          if (walletBalance === 0) { setShowTopUpAlert(true); return; }
                          setPoints((p) => Math.min(walletBalance, p + 10));
                        }}>+</button>
              </div>
              {points > walletBalance && (
                <p style={{ marginTop: 6, fontSize: 12, color: "#dc2626" }}>
                  You only have {walletBalance.toLocaleString("en-US")} pts.{" "}
                  <button type="button" style={{ color: "#059669", textDecoration: "underline", background: "none", border: "none", cursor: "pointer", fontSize: 12, padding: 0 }}
                    onClick={() => { onClose(); window.dispatchEvent(new CustomEvent("baylo:openWallet")); }}>
                    Top up →
                  </button>
                </p>
              )}
        ...
        <div className="modal-actions">
          ...
          <button className="btn-accent" onClick={submit}
            disabled={loading || (offerType !== "item" && points > walletBalance)}>
      ...
      {showTopUpAlert && (
        ...
        >
          ...
          >
            <div style={{ fontWeight: 700, fontSize: 16, color: "#064e3b", marginBottom: 10 }}>Not enough Baylo points</div>
            <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.6, marginBottom: 22 }}>
              You have 0 Baylo points. Top up your wallet to add points to your offer.
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                onClick={() => { setShowTopUpAlert(false); onClose(); window.dispatchEvent(new CustomEvent("baylo:openWallet")); }}
              >
                Top up
              </button>
            </div>
...
// Eco-Points reward system checklist — reward system, not gamification:
// wording is strictly "Eco-Points" and "tasks".
const ECO_TASK_UI: Record<EcoTaskKey, { label: string; repeatable: boolean; hint?: string }> = {
...
function EcoTasksCard() {
  const { ecoTasks } = useDash();
  const rank = getEcoRank(ecoTasks.ecoPoints);
  return (
    <div className="card railcard ecotasks">
      <div className="railcard-head">
        <h3><Icon name="leaf" size={16} fill="x" />Eco-Points</h3>
        <span className="ecotasks-total">{ecoTasks.ecoPoints} pts</span>
      </div>
      <p className="ecotasks-rank">
        <strong>{rank.label}</strong>
        {rank.next ? ` — ${rank.next.ptsToNext} pts to ${rank.next.label}` : " — top rank"}
      </p>
      <p className="ecotasks-sub">Earn Eco-Points by completing tasks.</p>
      <ul className="rail-list ecotasks-list">
        {ecoTasks.tasks.map((t) => {
          const ui = ECO_TASK_UI[t.task];
          const showHint = t.task === "VERIFY_ACCOUNT" && !t.done && !ecoTasks.googleVerified;
          return (
            <li key={t.task} className={"ecotask-row" + (t.done ? " done" : "")}>
              <span className="ecotask-check" aria-hidden="true">
                {t.done && <Icon name="check" size={12} />}
              </span>
              <div className="ecotask-body">
                <span className="ecotask-label">
                  {ui.label}
                  {ui.repeatable && t.count > 1 && <em className="ecotask-count">×{t.count}</em>}
                </span>
                {showHint && <span className="ecotask-hint">{ui.hint}</span>}
              </div>
              <span className="ecotask-pts">
                {t.done ? `+${t.pointsEarned}` : `+${ECO_TASK_POINTS[t.task]}`} pts
              </span>
            </li>
          );
        })}
...
export default function BayloDashboard({
  user, impactData, ecoTasks, trendingTags, activeTrades, messages, notifications,
  ...
  const dashCtx = React.useMemo<DashContextValue>(() => ({
    ...
    setFollowingEntry,
    openConfirmSwap,
  }), [user, activeTrades, feedPosts, stories, matchesProp, categoryCounts, weeklyTrades, impactData, ecoTasks, trendingTags, liveFollowingMap, setFollowingEntry, openConfirmSwap]);
