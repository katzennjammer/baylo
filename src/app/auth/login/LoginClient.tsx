...
function PointsSwatch({ amount }: { amount: number }) {
  return (
    <span style={{
      width: 42, height: 42, borderRadius: 11, flexShrink: 0,
      ...
      <span style={{ fontFamily: "var(--ff)", fontWeight: 700, fontSize: 8, color: "var(--accent)", lineHeight: 1, letterSpacing: "0.06em" }}>
        PTS
      </span>
    </span>
  )
        ...
        <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", gap: 12 }} aria-hidden>
          {swaps.length === 0 ? (
            ...
            <div key={s.id} style={{
              ...
              {(() => {
                const isPointsOnly = (s.offeredPoints ?? 0) > 0
                const leftLabel = isPointsOnly ? `${s.offeredPoints!.toLocaleString()} pts` : s.itemA.title
                const titleStr  = isPointsOnly
                ...
                return (
                  <>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      {isPointsOnly ? (
                        <PointsSwatch amount={s.offeredPoints!} />
                      ) : (
