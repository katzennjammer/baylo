...
export default async function ItemDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.item.findUnique({
    ...
    include: {
      user: {
        select: { id: true, name: true, avatar: true, rating: true, totalTrades: true, ecoPoints: true, location: true, createdAt: true },
      },
    },
  })
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
        ...
        <div>
          ...
          {item.estimatedValue && (
            <p className="text-sm text-gray-500 mb-4">Estimated value: <span className="font-semibold text-gray-700">₱{item.estimatedValue.toLocaleString()}</span></p>
          )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-2 flex-wrap">
                  ...
                  />
                  <EcoRankBadge ecoPoints={item.user.ecoPoints} size="sm" />
                </div>
              </div>
            </div>
          </div>
